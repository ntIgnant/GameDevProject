...
def load_walk_frames_boss():
    ...
    walk_path = os.path.join(ASSETS_DIR, "Characters", "Enemy", "boss3_walk.png")
...
def load_attack_frames_boss():
    attack_path = os.path.join(ASSETS_DIR, "Characters", "Enemy", "boss3_attack.png")
...
#This section controls the normal 3-bullet attack
    ...
    def _shoot_bullets(self, target_center):
        from Game.gun import HomingProjectile
        direction = pygame.Vector2(target_center) - self.pos
        
        # Safety check if target distance and boss distance are the same
        if direction.length_squared() == 0:
            return
        ...
        turn = 80 # How fast the homing bullets turn
        
        # For each bullet calculate its angle within the cone width
        for bullet in range(total_bullets):
            angle = -spread_angle/2 + spread_angle*bullet / (total_bullets -1)
            direction_bullet = direction.rotate(angle).normalize()
            spawn = self.pos + direction_bullet * (self.size //2 - 10)
            
            # If the bullet should follow the player, we create it as homing
            if bullet < homing:
                h_bullet = HomingProjectile(spawn.x, spawn.y, direction_bullet, turn)
                self.bullet_queue.append(h_bullet)
            
            else:
                self._make_bullet(direction_bullet)
    ...
    def update(self, dt, target_center, obstacles, player_rect=None, area_rect=None, speed_multiplier=1.0):
        ...
        if self.state == "attack":
            ...
            if not self.attack_fired and self.state_counter >= self.attack_windup:
                self._shoot_bullets(target_center)
        ...
        if self.special_timer >= current_special_interval and dist <= self.phase_two_distance:
